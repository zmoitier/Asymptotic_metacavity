...
def main():
    ...
    data_pla = np.loadtxt("../data/peanut_eCst_-1.1_pla")

    data_fem = np.loadtxt("../data/peanut_eCst_-1.1_8")
    k_fem = data_fem[:, 0]
    r_fem = data_fem[:, 1]
    ...
    ext = "pdf"
    fig.savefig(f"../images/peanut_eCst_norm.{ext}", transparent=True, format=ext)
    plt.close(fig=fig)

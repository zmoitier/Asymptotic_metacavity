...
Real inv_eps(const Point &P, Parameters &pa = defaultParameters)
{
  ...
  return 2 / (eMin * (1 - P(1)) + eMax * (1 + P(1)));
}
...
int main(int argc, char **argv)
{
  ...
  // Parameters
  Real eMin = atof(argv[1]);
  Real eMax = atof(argv[2]);
  Real k = atof(argv[3]);
  Int dGL = atoi(argv[4]);
  ...
  Parameters params;
  params << Parameter(eMin, "eMin") << Parameter(eMax, "eMax") << Parameter(M, "M") << Parameter(k, "k") << Parameter(T, "T");
  ...
  BilinearForm auv = intg(Cavity, Function(inv_eps, params) * grad(u) | grad(v)) +
